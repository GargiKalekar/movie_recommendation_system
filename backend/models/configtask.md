# MERN Task Manager Deployment — AWS EC2
...
On WSL, copy key first:

```bash
cp /mnt/c/Users/<your-path>/mern-key.pem ~/
chmod 400 ~/mern-key.pem
```
...
cd cc-practical-repos/mern-task-manager
```
...
Test from your laptop (new terminal):

```bash
curl http://<EC2-PUBLIC-IP>:5000/api/tasks
```

Test POST:

```bash
curl -X POST http://<EC2-PUBLIC-IP>:5000/api/tasks \
-H "Content-Type: application/json" \
-d '{"title":"Test Task","description":"Sample task","priority":"High","status":"Pending"}'
```
...
```bash
VITE_API_URL=http://<EC2-PUBLIC-IP>:5000/api/tasks npm run build
```
...
```
node server.js  → testing only
pm2 start       → final/production

localhost       → the VM itself, not your laptop

ALWAYS add swap before npm install
```
...
| Username | azureuser | ubuntu |
| Open ports | Inbound port rules | Security Group inbound rules |
| Restart VM | Stop/Start in Portal | Stop/Start in EC2 Console |
